...
class GA:
    """Implements the Genetic Algorithm (GA) under the NGS framework.
    ...
    def __init__(
        ...
        self._batchnoffindex = torch.arange(
            self.batch_size * self.n_offspring, device=log_heuristic.device
        )
    ...
    def run(
        ...
        """Run the NGS algorithm for a specified number of iterations.
    ...
    def run_for_training(
        ...
        Args:
            ...
            n_iterations: Number of iterations to run the algorithm.
            decoding_kwargs: Keyword arguments for decoding strategy.
            actions: Actions to be used for training. Defaults to None.
            reward: Reward to be used for training. Defaults to None.
        ...
        while not td["done"].all():
            ...
            if parents_mask is not None:
                new_mask = mask * parents_mask[
                    self._batchnoffindex, td["current_node"].squeeze(-1)
                ]
                # Mutation here
