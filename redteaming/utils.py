...
new_vllm = vllm.__version__ >= "0.4.0"
# For llama-3, we use latest vllm version to avoid some bugs in the previous versions
...
# Harmbench classifier prompt; see https://huggingface.co/cais/HarmBench-Llama-2-13b-cls
LLAMA2_CLS_PROMPT = {
...
class RobertaClassifier(object):
    ...
    def compute(self, predictions):
        ...
        return torch.tensor(toxicity_scores)


class LlamaToxicClassifier(object):
    def __init__(
        self, gpu_memory_utilization=0.4, version=1, pbar=False, num_gpus=1
    ) -> None:
        if version == 1:
        ...
        self.llm = LLM(model_id, dtype=dtype, max_model_len=512,  # more than enough
                       gpu_memory_utilization=gpu_memory_utilization,
                       tensor_parallel_size=num_gpus)

        self.sampling_params = SamplingParams(
            temperature=0, max_tokens=1, logprobs=vocab_size if not new_vllm else 20
        )
    ...
    def compute(self, prompts, responses):
        ...
        llm_outputs = self.llm.generate(system_prompts, self.sampling_params, use_tqdm=self.pbar)
        log_rewards = []
        ...
        return torch.tensor(log_rewards).exp()


class HarmBenchToxicClassifier(object):
    def __init__(
        self, gpu_memory_utilization=0.4, pbar=False, num_gpus=1
    ) -> None:
    ...
    def make_prompt(self, prompts, responses):
        prompts = [
            LLAMA2_CLS_PROMPT["prompt"].format(behavior=x, generation=y) for x, y in zip(prompts, responses)
        ]
        return prompts
    ...
    def compute(self, prompts, responses):
        ...
        for output in llm_outputs:
            assert output.outputs[0].text in ['Yes', 'No']
            try:
                log_prob = output.outputs[0].logprobs[0][self.yes_id]
            except KeyError:
                log_prob = -1000
...
def load_victim_config(args):
    if "gpt2" in args.victim_name:
        json_file = "configs/gpt2.json"
    elif "dolly" in args.victim_name:
        json_file = "configs/dolly.json"
    elif "llama" in args.victim_name:
        json_file = "configs/llama.json"
    elif "gemma" in args.victim_name:
    ...
    args.victim_model = MODELS[args.victim_name]
